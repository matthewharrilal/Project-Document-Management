...
class ViewController: UIViewController {
    ...
    @IBAction func unzipButton(_ sender: Any) {
        DispatchQueue.main.async {
            self.instance.getCollections(route: .forest(), requestRoute: .get) { (url) in
                ...
                let unzip = try? Zip.unzipFile(url!, destination: cachesURL!, overwrite: true, password: "password", progress: { (progress) in
    ...
    @IBAction func swimmingButton(_ sender: Any) {
        self.instance.getCollections(route: .swimming(), requestRoute: .get) { (url) in
            ...
            let unzipDirectory = try? Zip.unzipFile(url!, destination: cacheURL!, overwrite: true, password: "password", progress: { (progress) in
